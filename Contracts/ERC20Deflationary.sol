...
contract ERC20Deflationary is Context, IERC20, Ownable {
    ...
    */
    ...
    function setTaxLiquify(uint8 taxLiquify_) public onlyOwner {
     ...
     /**
      * @dev Collects ETH dusts. 
      * Dust/leftover (usually in very small quantity)
      * comes from add to liquidity. The rate(price) of this token
      * might fluctuate between swapping and add to liquidity pool.
      * When that happens, there would be some small quantity of ETH or tokens
      * left from pairing. 
      *
      */
    function collectDusts() public onlyOwner {
        require(payable(_msgSender()).send(address(this).balance), "Collect failed.");
    }
